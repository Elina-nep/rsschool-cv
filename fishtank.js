...
class Fish {
    ...
    angleFromFishDirectionToClosestFish(otherFish) {
        otherFish = otherFish == null ? this.following : otherFish;
        if (otherFish) {
    ...
    angleDirectionDifference(otherFish) {
        otherFish = otherFish == null ? this.following : otherFish;
    ...
    calc() {
        ...
        if (this.following == null || py(this.x - this.following.x, this.y - this.following.y) > FOLLOW_DISTANCE) {
            if (this.following != null) {
                // if (keyDown)
                //     affinityLine(this.following, this, "white");
            ...
            for (let i = 0; i < fishes.length; i++) {
                let fish = fishes[i];
                if (fish != this) {
                    let distance = py(this.x - fish.x, this.y - fish.y);
        ...
        if (this.following != null) {
            ...
            if (this.angleDirectionDifference() > (Math.PI * 0.9) && // On colliding angle?
                ...
                // if (keyDown)
                //     affinityLine(this.following, this, "yellow");
            ...
            // if (keyDown)
            //     affinityLine(this.following, this, "red");
        }
        ...
        // Poor little fishies love your cursor
        if (py(this.x - (cursor.x), this.y - (cursor.y)) > FOLLOW_DISTANCE * 2) {
        ...
        if (this.following != null) {
            for (let i = 0; i < this.following.entourage.length; i++) {
                let siblingFish = this.following.entourage[i];
                if (siblingFish !== this) {
        ...
        let speed = Math.max(0.1, Math.min(maxSpeed, py(this.dx, this.dy)));
...
//Draw Circle
function draw(f) {
    let r = f.angle + Math.PI;

    context.translate(f.x, f.y);
    context.rotate(r);

    let w = 20;
    let acc = py(f.dx - f.ox, f.dy - f.oy) / 0.05;
    ...
    context.drawImage(fishBitmap, 0, 0, w, 5);
    context.rotate(-r);
    context.translate(-f.x, -f.y);
}
...
document.onmousemove = function (e) {
    ...
    else if (e.pageX < 0) { cursor.x = 0 }
    else { cursor.x = e.pageX; }

    if (e.pageY > window.innerHeight) {
        cursor.y = e.pageY - window.pageYOffset
    }
    else if (e.pageY < 0) { cursor.y = 0 }
    else { cursor.y = e.pageY; }
...
const deltaAngle = (f, o) => { //Find the shortest angle between two
    const r = f - o;
...
const update = () => {
    ...
    // thanks to https://codepen.io/gadgetgnome/pen/qEWNGK for animation idea and code